...
if (!STARKNET_RPC_URL) throw new Error("Manca STARKNET_RPC_URL nel .env");

// Scegli quali file processare
const SIZES = [16];
...
(async function main() {
  ...
  for (const size of SIZES) {
    ...
    if (!fs.existsSync(inFile)) {
      console.warn(`(skip) manca ${inFile}`);
      continue;
    }
    ...
    out.push(`File sorgente: l2_batch_${size}.txt`);
    out.push("");
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        if (l1Block || l1TxHash) {
          ...
          console.log("nessun campo L1 esposto dall'RPC");
        }
